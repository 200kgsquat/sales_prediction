...
sale_schema = DataFrameSchema(
    {
        "date": Column(
            pa.String,
            Check(_is_valid_date, element_wise=False, error="Invalid date format: expected DD-MM-YYYY or YYYY-MM-DD"),
            nullable=False,
            coerce=True,
        ...
        Check(lambda x: x <= 300000, error="item_price must be <= 300000")
        ],
        nullable=False,
        coerce=True,
        description="Item price: >0 and <=300000"
        ),
        "item_cnt_day": Column(
        pa.Float,
        [
        Check(lambda x: x >= 0, error="item_cnt_day must be >= 0"),
        Check(lambda x: x <= 30000, error="item_cnt_day must be <= 30000")
        ],
        nullable=False,
        coerce=True,
        description="Items sold: >=0 and <=30000"
        ),
